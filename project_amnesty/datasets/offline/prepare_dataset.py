...
Inputs are the ~~Dataset classes in project_amnesty.datasets — for each entry under
...
def build_group(group: str, builders: list[BaseDataset], out_root: Path,
    ...
    the concurrency gate live in the sibling prepare.py, so that the plain
    config-driven CLI and the ensure-prepared path share exactly this body and
    cannot drift.
    """
