...
class MoshiKDDataset(Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        assert self.path.exists(), (
            ...
            f"    python -m project_amnesty.datasets.prepare --group {self.data_dir}"
        )
    ...
    def _pick_ref(self, item: KDSample, row: int, rng: np.random.Generator) -> dict:
        ...
        if self.sample_type == "en_kd":
            ...
            # project_amnesty writes "". Asserted rather than assumed: if a future
...
def build_source_datasets(
    ...
    assert root.exists(), (
        ...
        f"    python -m project_amnesty.datasets.prepare --group <source>"
    )
