"""Ensure-materialized gate over the offline builders -- and the prepare-only CLI.
    ...
    python -m training.datasets.prepare --group en_kd
...
``data_pipeline.prepare_dataset.builders_for_group``.
"""
...
from data_pipeline.prepare_dataset import (
...
def _check_dependencies(group: str, root: Path) -> None:
    for dep in GROUP_DEPENDENCIES.get(group, ()):
        ...
        raise FileNotFoundError(
            ...
            f"    run first: python -m training.datasets.prepare --group {dep}"
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--all", action="store_true",
                   help="prepare every group registered in data_pipeline.datasets.REGISTRY")
