...
**범위**: 런타임 레이어 설계 (Dataset / Collator / Sampler / Config).
**비범위**: 학습 루프, 손실 함수 구현, 모델 코드.

> 이 문서는 `training/data/`를 전제로 작성되었다. 구현은 `project_amnesty/datasets/`에
> 평탄화되어 들어갔고(오프라인 빌더와 같은 패키지), 본문의 경로 표기와 `file:line` 앵커는
> 그 이전 것이다. 설계 근거는 유효하되 경로는 현재 트리를 따를 것.

오프라인 준비(raw → Arrow)는 같은 패키지의 `base.py` / `*_dataset.py` / `prepare_dataset.py`가
담당하고, 이 문서는 그 산출물을 `torch.utils.data` 표준 계약으로 소비하는 레이어를 정의한다.
오프라인 쪽은 참고 대상이지 따라야 할 규격이 아니다. 규격은 `Dataset` / `Sampler` /
`collate_fn` 3-피스 계약이다.
...
| 항목 | 값 | 출처 |
|---|---|---|
| Mimi 상수 | 12.5Hz, K=8, card=2048, 24kHz | [schema.py:27-30](../../project_amnesty/datasets/schema.py#L27-L30) |
| 준비 데이터 경로 | `data/prepared/{source}/{split}/` (HF `save_to_disk`) | [prepare_dataset.py:49](../../project_amnesty/datasets/prepare_dataset.py#L49) |
| group | `en_kd` / `en_solo` / `ko_tts` / `text_anchor` | [base.py:20](../../project_amnesty/datasets/base.py#L20) |
| split | `train` / `probe` — probe는 **절대 학습 금지** | [prepare_dataset.py:12](../../project_amnesty/datasets/prepare_dataset.py#L12) |
| 저장 레이아웃 | **delay-free, frame-aligned**. delay는 collator가 적용 | [en_kd_dataset.py:281](../../project_amnesty/datasets/en_kd_dataset.py#L281) |
| A/B 대칭 | "누가 user인지" 미저장. 학습 시점 결정 → 2× 데이터 | [schema.py:12](../../project_amnesty/datasets/schema.py#L12) |
| teacher logits | raw top-k. **temperature는 loss 하이퍼파라미터** | [schema.py:14](../../project_amnesty/datasets/schema.py#L14) |
...
1. `source ≠ sample_type`. `en_solo`는 `source="en_solo"`이지만 `sample_type="ko_tts"`
   ([en_solo_dataset.py:57-59](../../project_amnesty/datasets/en_solo_dataset.py#L57-L59)).
...
`find_solo_windows` ([en_solo_dataset.py:26](../../project_amnesty/datasets/en_solo_dataset.py#L26))가
...
([en_kd_dataset.py:306](../../project_amnesty/datasets/en_kd_dataset.py#L306))을 **solver가 아니라
...
- `MoshiSelfTalkEngine`이 `NotImplementedError` 스텁
  ([en_kd_dataset.py:112](../../project_amnesty/datasets/en_kd_dataset.py#L112)) →
...
## 9. 데이터 준비 계층 — `project_amnesty` 변경안
...
1. **N:1 관계.** [prepare_dataset.py:60-68](../../project_amnesty/datasets/prepare_dataset.py#L60-L68)에서
...
[base.py:24-26](../../project_amnesty/datasets/base.py#L24-L26)의 docstring이 이미 둘이 다르다고
말하고 있지만 이름이 그걸 가린다.
...
```python
# project_amnesty/datasets/prepare_dataset.py
...
([mixins.py:167](../../project_amnesty/datasets/mixins.py#L167))가 인코딩을 건너뛰고,
...
| 상태 | 팀 fork의 streaming 인터페이스 확정 대기 | **이미 동작** ([en_kd_dataset.py:277](../../project_amnesty/datasets/en_kd_dataset.py#L277)) |
| 규모 확장 | 자동 | 수동 |
...
**D4: `voice_prompt_frames = 48`** (3.84초). 폐기된 VQ-프롬프트 시나리오 실험의 `REF_FRAMES`와
동일 — 이 레포에서 실제로 클로닝을 끝까지 돌려본 값이다. 기존 100(8초)은 실제
