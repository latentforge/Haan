...
- 임베딩 초기화는 Moshi의 코드북 RVQ 임베딩 값을 사용하되, **Moshi의 user 쪽 테이블(`emb.8~15`)에서 복사**한다 — user 쪽이 다화자 분포에서 학습돼 고정 화자 편향이 없기 때문(근거·상세는 §5.4.2). 공유 임베딩 구조 자체는 §2~3에서 논의한다.
...
1. **role 구분 신호의 강건성**: role token은 학습되는 파라미터라, 다른 loss 압력에 밀려 role 구분 신호가 약해지거나 다른 정보와 얽힐 이론적 위험이 있다. 학습 후 self/user role 벡터의 분리도(코사인 유사도)와, user 콘텐츠가 실제로 self와 구분되어 처리되는지를 프로빙으로 점검한다.
   - **우선 점검 대상은 semantic 레벨이다** (아래 §3.5.1 실측 근거). role 구분이 semantic에 몰려 있으므로 Role Token이 실제로 부담을 지는 지점도 거기다.
...
#### 3.5.1 실측: 원 Moshi에서 role 구분은 어디에 있나

`kyutai/moshiko-pytorch-bf16` 가중치에서 self/user 임베딩 테이블을 직접 비교한 결과 (`tools/inspect_moshi_weights.py`):

| | codebook 0 (semantic) | codebook 1 (acoustic) |
|---|---|---|
| row cosine(self_i, user_i) | **0.501** | **0.751** |
| 상수 오프셋이 설명하는 분산 | 1.76% | 7.82% |
| 코드별(잔차) 비중 | 98.24% | 92.18% |

두 가지 함의:

1. **role 신호는 깔끔한 additive 오프셋이 아니다.** 코드 무관한 일정 방향이 설명하는 분산은 1.8~7.8%에 불과하고 90%+가 코드별로 제각각이다. 즉 원 Moshi의 role 구분은 "공통 표현 + role 바이어스"가 아니라 **테이블 전체가 다른 표현**으로 인코딩돼 있다. (독립 random init에서 출발해 코사인 0.50~0.75까지 수렴한 것이므로, 공유 구조와 role 특화가 함께 학습된 결과로 읽힌다.) → **"role 오프셋을 추출해 다른 테이블에 얹는" 방식은 근거가 없다.**
2. **role 분화는 semantic에 집중된다.** acoustic은 self/user가 이미 상당히 유사(0.75)하고 semantic이 확연히 갈린다(0.50). 저수준 음향 텍스처는 역할과 무관하지만 "무슨 말인가"는 화자 역할이 중요하다는 해석과 정합적이다.

> **정직한 confound**: acoustic 코드북은 loss 다운웨이팅(non-semantic ×0.02, §7.6)으로 그래디언트를 적게 받으므로, 높은 유사도가 "역할 무관성"이 아니라 **분화 압력 부족**을 반영했을 수 있다. acoustic 레벨 1→7로 갈수록 유사도가 단조 증가하는지 확인하면 두 해석을 가를 수 있다(저비용 후속 측정).
...
**릴리스 체크포인트 실측 텐서 구성** (`kyutai/moshiko-pytorch-bf16`, safetensors 헤더 직접 확인, 총 355 텐서):

| 텐서 | 개수 / shape | 의미 |
|---|---|---|
| `emb.0~15` | 16 × `[2049, 4096]` | 오디오 입력 임베딩 — **self 8 + user 8 완전 분리**(§2.1 확증) |
| `depformer_in.0~7` | 8 × `[1024, 4096]` | 백본→depformer 프로젝션이 **depth step k마다 별도** |
| `depformer.layers.0~5` | 6층 × 20텐서 | `gating.0~7`, `self_attn.in_proj [24576,1024]`(=3×8×1024), `out_proj [8192,1024]`(=8×1024) → **모든 선형층이 코드북 index별 ×8** |
| `linears.0~7` | 8 × `[2048, 1024]` | 오디오 출력 헤드 — **8개뿐, user 예측 헤드 부재**(§5.0.3) |
| `depformer_emb.0~6` / `depformer_text_emb` | 7 × `[2049,1024]` / `[32001,1024]` | depth step의 이전 토큰 임베딩 |
| `text_emb` / `text_linear` | `[32001,4096]` / `[32000,4096]` | Helium 텍스트 vocab |

> **주의 — "8"은 role 축이 아니라 코드북 step 축이다.** Depth Transformer는 **하나**이며, 그 파라미터가 step $k$로 인덱싱될 뿐이다(논문: *"different parameters per index k... different sub-sequences might require different transformations"*). Haan은 이 per-$k$ 구조를 **그대로 유지**하고, 새로 추가하는 **role 축만 additive로 공유**한다:
> $$z^{\text{depth}}_{k,\text{role}} = \text{depformer\_in}_k(z_s) + \text{RoleEmb}[\text{role}]$$
> 즉 프로젝션은 $k$별 8개이고 각각이 두 role에 공유된다. batch-2는 동일 파라미터에 self·user 두 입력을 병렬로 흘리는 것이다.

**Warm-start 처리**:

| 구성요소 | 처리 |
|---|---|
| Depth Transformer 본체 (per-index 포함) | **Moshi에서 warm-start** (1024 공간, 백본 무관) |
| `linears.0~7`, `depformer_emb.0~6` | **Moshi에서 warm-start** (동일 Mimi, card 2048) |
| **오디오 입력 임베딩 (공유 8개)** | **Moshi의 `emb.8~15`(user 쪽)에서 복사** ← §5.4.2 |
| `depformer_in.0~7` (4096→1024) | **shape 호환** → Moshi 가중치로 초기화하되 **재학습**. Helium과 Qwen3의 4096 공간은 정렬돼 있지 않으므로(§5.1 hidden-state KD 폐기와 동일 논리) 어댑터로서 학습이 필요하다 |
| Role embedding (1024×2) | 신규, 매우 작음 |
| 텍스트 임베딩 / 헤드 (`text_emb`, `text_linear`, `depformer_text_emb`) | 전이 불가(32000 vs 151936) → Qwen3 자체 사용 |

#### 5.4.2 오디오 입력 임베딩은 Moshi의 **user 쪽**(`emb.8~15`)에서 초기화한다

Haan은 self/user 공유 임베딩 8개 테이블을 쓰는데(§3.3), 그 초기값을 Moshi의 16개 중 **user 쪽 8개(`emb.8~15`)**에서 복사한다.

**근거**: Moshi의 두 테이블은 서로 다른 화자 분포를 보고 학습됐다.
- `emb.0~7`(self): 마지막 instruct 튜닝에서 **단일 배우 목소리**로 좁혀짐(§4.3 — *"To give Moshi its own consistent voice, we condition the TTS engine on the voice of a single actor"*).
- `emb.8~15`(user): *"the voice of the second audio stream (the user) is **randomly sampled for each example**, giving more robustness to different speaking conditions and accents"* → **다화자 분포에서 학습**.

고정 화자 편향 제거가 본 연구의 출발점이므로, **한 번도 단일 음색에 묶인 적 없는 user 쪽 표현에서 출발**하는 것이 voice-prompt 기반 클로닝(§5.2)의 사전분포로 더 적합하다. 두 테이블 모두 동일한 Mimi 코드(card 2048)를 인덱싱하므로 복사에 구조적 문제는 없다.
...
- **Temporal vs Depth 구분(중요)**: Temporal은 프레임 문맥 $z_s$를 만들고, Depth는 그 조건 위에서 프레임 내 코드북을 생성한다(§5.0). **Haan에서는 Depth가 self·user 두 스트림을 모두 생성**하므로(§5.4, 공유 Depth + role embedding + batch-2), 임베딩 공유는 **인식(Temporal)과 생성(Depth) 양쪽 모두**에 기여한다. semantic 공유는 Temporal의 user 인식에, acoustic 공유는 Depth의 다화자 음향 생성에 직접 작용한다. 실측상 role 분화는 semantic에 집중돼 있어(§3.5.1) **acoustic 공유는 잃는 role 정보가 적고**, 따라서 semantic/acoustic 모두 공유한다. 공유 범위별 효과는 §3.6·6.2 ablation에서 정량화한다.

### 6.2 논문 1: Role Token — 한국어 전이를 위한 임베딩 통합 (본 트랙 채택안)
- **컨트리뷰션**: Moshi의 self/user 오디오 임베딩 분리가 저자원 언어 전이를 저해함을 규명하고, 임베딩을 완전 공유하되 role 구분을 이진 role token(학습형 additive 벡터, self/user 각 1개)으로 대체. 단일 화자 발화(비멀티턴) 데이터만으로 이해(user 인식)와 생성(Haan에서는 Depth가 self·user 양쪽을 생성, §5.4) 모두에 한국어 능력이 전이됨을 실증.
- **설계**: 오디오 임베딩 테이블 self/user 공유(semantic+acoustic 전체), role 구분은 `RoleEmb[role_id]`(0=self,1=user) additive, 시간축 RoPE는 그대로 유지. (§3.3과 동일) 공유 테이블 초기값은 Moshi의 **user 쪽 `emb.8~15`**에서 복사(§5.4.2).
- **핵심 리스크 (§3.5.1 실측 근거)**: 원 Moshi는 role을 **코드별로 제각각인 비-additive 방식**으로 인코딩한다(상수 오프셋 설명력 1.76%/7.82%, 잔차 90%+). 그런데 Role Token은 그것을 **단일 additive 벡터**로 대체하므로, 표현력 관점에서는 **원 설계보다 약한 메커니즘**이다. 즉 "공유해도 role 구분이 유지되는가"는 낙관할 사안이 아니라 ablation (1) vs (2)가 정면으로 검증해야 할 핵심 질문이다. §3.5의 프로빙(특히 semantic 레벨)이 이 리스크의 조기 경보 역할을 한다.
...
- **Ablation**: (1) 원 Moshi 완전 분리(baseline) (2) 공유+Role Token(제안) (3) 공유+Dual PE 대안 (4) semantic만 공유 vs semantic+acoustic 모두 공유 (5) 코드북 init: user(`emb.8~15`, 채택) vs self(`emb.0~7`) vs random.
  - **사전 예측(§3.5.1)**: acoustic은 원 Moshi에서도 self/user가 이미 유사(cos 0.75)하므로 **공유 시 잃는 role 정보가 적다**. 반면 semantic은 확연히 갈리므로(cos 0.50) **(4)의 실질 시험대는 semantic 공유**다. (1) vs (2)의 격차도 semantic에서 가장 크게 나타날 것으로 예상.
...
| 출발점 | Moshi warm-start(§5.4.1) + Qwen3 백본, 한국어 데이터 | 원 Moshi에서 독립 시작 가능 |
