...
export class MockEventSource {
  constructor(script = advisorDemoScript) {
    ...
    this._onEvent = null;
    this._seq = 0;
    // 데모 세션 식별자. 실서비스에선 서버가 발급하지만, Mock 은 자체 생성한다.
    this.sessionId = `demo-${Date.now().toString(36)}`;
  }

  // 공통 봉투(session_id/seq/ts)를 씌운다 — 계약: SERVICE_ARCHITECTURE.md §6.
  // 실제 서버는 오케스트레이터가 봉투를 붙이지만, 여기서는 Mock 이 대신한다.
  _wrap(event) {
    return { session_id: this.sessionId, seq: ++this._seq, ts: new Date().toISOString(), ...event };
  }
  ...
  async _play() {
    ...
    for (const item of this.script) {
      ...
      if (this._onEvent) await this._onEvent(this._wrap(event));
    }
  }
}
