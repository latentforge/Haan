...
(function () {
  ...
  function connect() {
    ...
    ws.onopen = function () {
      ...
      // user.html 은 별도의 "통화 시작" 버튼 없이 로드 즉시 통화 중 화면을 보여준다.
      // 그 화면과 실제 세션 상태를 맞추기 위해 접속 시 call_start 를 함께 보낸다.
      // (서버의 startCall() 은 이미 active 세션이면 no-op 이라 재접속/재로드에도 안전.)
  ...
  function handle(evt) {
    switch (evt.type) {
      case "ai_step":
        ...
        break;
      case "tool_call":
        if (evt.state === "running") setProc(TOOL_LABEL[evt.tool] || "요청 처리 중");
        else if (evt.state === "done") setProc("처리 완료");
        else if (evt.state === "error") setProc("처리 실패 · 재시도 필요");
        break;
      case "transcript":
