...
class ToolRunner {
  ...
  /** 공통: tool_call 상태 전이 이벤트를 흘려보낸다. (계약: call_id, state) */
  _emitStatus(callId, tool, state, extra = {}) {
    this.emit({
      type: "tool_call",
      call_id: callId,
      tool,
  ...
  async verify_identity(callId, request) {
    ...
    // 잠금 해제 상태 전이(계약: verify_state). check="phone" 으로 상담사 UI 체크리스트를 채운다.
    // ⚠️ 잠금의 권위는 위의 session.unlocked(서버 상태)이며, 이 이벤트는 그 결과의 통지일 뿐이다(§4).
    this.emit({
      type: "verify_state",
      unlocked: true,
      customer_id: customer.customer_id,
