...
> - **Instruction template은 "정적 prefix = dense ChatML / 대화 구간 = PAD·EPAD" 2구역 설계**를 따른다. System Prompt를 Qwen 네이티브 형식으로 앞에 두고 assistant 블록을 열어둔 채, 대화 구간에는 턴 마커를 넣지 않는다(턴 경계 기능은 Moshi 네이티브 PAD/EPAD가 담당). `<|im_end|>`는 세션 종료에만 쓰고, PAD/EPAD는 예약 특수 토큰에 신규 배정한다. 라이브 추론은 non-thinking 모드로 운용해 텍스트 채널 12.5 tok/s 제약을 회피한다. (`ARCHITECTURE.md` §7)
